...
- **The fighters learn.** A throw is aimed high, at the body or low, and each guard answers exactly one of the three — duck a high one, block a body one on the disc, jump a low one. Nothing beats anything else outright, so there is no move to settle on: the only edge is in reading what the other side tends to do while not being read back. Each side carries two small policy networks, one for attacking and one for defending, shared by its fighters and trained on every throw by REINFORCE. Against a side with a habit they punish it hard — a defender that blocks 70% of the time is read inside a few hundred throws and sees 80% of throws get past, against 67% for guessing. Against each other they hold the honest answer, which is to stay unreadable. `discwars/learn.js`; the page shows what each side has settled on, and the scene still runs if that file is missing, just blind.
- **The fight plays itself.** A four-on-the-floor kick with a snare on two and four holds the beat underneath, and **every fighter knocked out of the round takes the tempo up ten** — 120 to as much as 170 as a 3v3 is worn down to a win, back to 120 when the next round starts, so the last exchanges of a round are the fastest. Tempo changes land on a bar line rather than wherever the fight puts them; every throw and every block fires a run of 7, 9, 11 or 13 sixteenths — played three, a breath, then the rest — walking the notes of D# natural minor less its fifth (D#, E#, F#, G#, B, C#) rather than semitones, so three lines at once still agree. **The length and the shape of the walk come from the fighter's own name**, so GREP throws the same figure every time and a champion carries its sound into the next round; the aim decides which degree the run starts on. The rest of the fight is the kit: a **crash** for every ring smashed out from under somebody, a **tom** for every bounce and every block, and which one is not random: a disc falls through the kit as it travels, high for the first thing it hits, middle for the second or for a shield turning it away, low for the last and for its arrival back in the hand. A throw off a wall, past its target, off the glass and home comes out as a descending fill that follows the disc, and a **ride** for every derez and the two sides an octave apart. Events are held to the next eighth rather than played when they happen, which is the whole difference between a techno track and a clatter — at 120 BPM that wait is never more than a quarter second, and the disc is still in the air either way. All oscillators and one buffer of noise, no samples (`discwars/sound.js`). Off until asked for, on the page's Sound control; the screensaver stays silent like Light Cycles.
- **Everyone has a name and a record.** Fighters are named from three pools — Unix commands, Greek given names, and languages that are also words (BASIC, FORTRAN, PASCAL, OBERON, SWIFT) — drawn without repeating, and the name is drawn over the head in the same stroke-vector style as the rest of the arena, shrinking with distance. Each keeps its own count of throws, hits, blocks, dodges, rings taken and kills, which comes to a points total.
- **A fighter remembers what worked.** A kill teaches two things, and the better of them is defensive: *"I jumped it, then killed with a body shot"* is a **counter**, kept against the guard it answered, so it comes back out when that situation comes round again. *"I opened high, then killed low"* is a **press**, an opening it has to start itself — kept too, but reached for half as often, because a pattern you begin yourself is the readable kind. Taking the ring out from under somebody already hanging is a third kind, a **finish**. Each pair is named with a verb and an animal (DIVING EEL, COILING OSPREY) and the champion's palette is listed under the scoreboard, counters marked. Only a champion keeps a palette; everyone else starts empty.
- **Defence and recovery are learned the same way.** A fighter that has hauled itself back over a ring edge before is better at it — a third of the time to begin with, rising toward three in five. One that has taken a banked shot out of the air is better at catching the next. And one that has answered a particular aim often enough can get a piece of a throw its guard did not cover at all, which is the save a fighter who has been here before makes and a new program does not.
- **And knowing a move makes you better at it.** A pair is not just what a fighter reaches for, it is what it is good at: a throw it has made before finds its mark more often (0.20 rising towards 0.36), and a finish it has done before is thrown without hesitating, the disc arriving in 0.85 s rather than 1.25 and leaving far less room to climb out of. The gain flattens off, so the tenth repetition is worth much less than the second and nobody becomes untouchable.
- **A champion is harder to put down.** Four things, all about not being killed rather than about killing: a throw that gets past its guard lands a third less often, the first hit each round that would break it apart only throws it back instead, its smashed rings rise again in half the time, and it is reeling for less of it after a knock-back. Memory turned out to be no help at surviving — four versions of it measured flat — because everything memory improves is about attacking, and a reign ends when the champion is killed.
- **One champion goes through.** A champion still standing at the end of a round keeps the place however the points fell — the title is held until somebody takes it off them, not lent out again each round to whoever had the best few minutes. Only when the champion has gone does it pass, and then to whichever survivor has most to show for the round. The champion carries its name and its whole record, and wears what it has held: a badge over its head that climbs the way enlisted insignia climbs — a chevron for the first round, a rocker under it for the second, more chevrons, then more rockers, and a lozenge and finally a star once there is nowhere else to go. Private to Sergeant Major in nine rounds, with the rank and the count on the scoreboard. everyone else is a new program with nothing to its name. So the champion is the only thing in the arena that accumulates, and the only one with anything to lose.
- A throw may instead go **off a side wall** — the glass across the arena, or the one between the fighters and whoever is watching, which is not drawn but is just as hard — and come in at an angle rather than straight on. One that is then dodged or turned away bounces off the opposite wall on its way home.
